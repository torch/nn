...
#define TH_GENERIC_FILE "generic/VolumetricBatchNormalization.c"
#else

void THNN_(VolumetricBatchNormalization_updateOutput)(THNNState *state, THTensor *input, THTensor *output, THTensor *weight, THTensor *bias, THTensor *running_mean, THTensor *running_var, THTensor *save_mean, THTensor *save_std, bool train, double momentum, double eps)
{
  long nBatch = THTensor_(size)(input, 0);
  long nFeature = THTensor_(size)(input, 1);
  long iT = THTensor_(size)(input, 2);
  long iH = THTensor_(size)(input, 3);
  long iW = THTensor_(size)(input, 4);
  long n = nBatch * iT * iH * iW;
...
void THNN_(VolumetricBatchNormalization_backward)(THNNState *state, THTensor *input, THTensor *gradOutput, THTensor *gradInput, THTensor *gradWeight, THTensor *gradBias, THTensor *weight, THTensor *save_mean, THTensor *save_std, double scale)
{
  long nBatch = THTensor_(size)(input, 0);
  long nFeature = THTensor_(size)(input, 1);
  long iT = THTensor_(size)(input, 2);
