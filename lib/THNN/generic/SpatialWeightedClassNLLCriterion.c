...
#define INITIAL_WEIGHTS_CHECK                                                      \
  {                                                                                \
    long weights0 = THTensor_(size)(weights, 0);                                   \
    long weights1 = THTensor_(size)(weights, 1);                                   \
    long weights2 = THTensor_(size)(weights, 2);                                   \
    long target0 = THIndexTensor_(size)(target, 0);                                \
    long target1 = THIndexTensor_(size)(target, 1);                                \
    long target2 = THIndexTensor_(size)(target, 2);                                \
    THAssertMsg(weights0 == target0 && weights1 == target1 && weights2 == target2, \
              "size mismatch (got weights: %ldx%ldx%ld, target: %ldx%ldx%ld)",     \
              weights0, weights1, weights2, target0, target1, target2);            \
  }
          ...
          THTensor *total_weight)
{
  ...
  for (int b = 0; b < batch_size; b++) {
    for (int elem = 0; elem < map_size; elem++) {
      ...
      real cur_weight = weights ? weights_data[b * map_size + elem] : 1.0f;
...
#pragma omp parallel for
  for (int b = 0; b < batch_size; b++) {
    for (int elem = 0; elem < map_size; elem++) {
      ...
      gradInput_data[b * sample_size + cur_target * map_size + elem] =
        -(weights ? weights_data[b * map_size + elem] : 1.0f) / normalize;
    }
  }
