...
#define TH_GENERIC_FILE "generic/WeightedClassNLLCriterion.c"
#else
...
#define INITIAL_WEIGHTS_CHECK                                   \
  {                                                             \
    ...
    THAssertMsg(weights0 == target0,                            \
              "size mismatch (got weights: %ld, target: %ld)",  \
              weights0, target0);                               \
  }

void THNN_(WeightedClassNLLCriterion_updateOutput)(
          THNNState *state,
          THTensor *input,
          ...
          THTensor *total_weight)
{
  ...
  if (THTensor_(nDimension)(input) == 1) {
    ...
    total_weight_data[0] = weights ? weights_data[0] : 1.0f;
...
void THNN_(WeightedClassNLLCriterion_updateGradInput)(
          THNNState *state,
          THTensor *input,
          THIndexTensor *target,
          THTensor *gradInput,
          ...
          THTensor *total_weight)
{
  ...
  if (THTensor_(nDimension)(input) == 1) {
    ...
    gradInput_data[cur_target] =
      (!sizeAverage && weights) ? -weights_data[0] : -1;
