          ...
          int padH)
{
  ...
  if(input->nDimension == 3)
  {
    THNN_(SpatialDepthWiseConvolution_updateGradInput_frame)(gradInput, gradOutput,
						      weight, fgradInput,
						      kW, kH, dW, dH, padW, padH);
  }
  else
...
#pragma omp parallel for private(t)
    for(t = 0; t < T; t++)
    {
      ...
      THNN_(SpatialDepthWiseConvolution_updateGradInput_frame)(gradInput_t, gradOutput_t,
							weight, fgradInput_t,
							kW, kH, dW, dH, padW, padH);
          ...
          real scale)
{
  ...
  if(input->nDimension == 3)
  {
    THNN_(SpatialDepthWiseConvolution_accGradParameters_frame)(gradOutput, gradWeight,
  ...
  else
  {
    ...
    for(t = 0; t < T; t++)
    {
      ...
      THNN_(SpatialDepthWiseConvolution_accGradParameters_frame)(gradOutput_t, gradWeight,
