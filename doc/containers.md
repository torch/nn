...
It reimplements many of the Module methods such that calls are propagated to the contained modules.
For example, a call to [zeroGradParameters](module.md#nn.Module.zeroGradParameters) will be propagated to all contained modules.

<a name="nn.Container.add"></a>
### add(module) ###
Adds the given `module` to the container.
...
<a name="nn.Container.prettyPrint"></a>
### prettyPrint([status]) ###
If there is no `status` argument, this function toggles the output color option, which is `false` by default.
Otherwise, sets the output color option to `status`, which can be `true` or `false`.
By color coding different `Container`s, helps navigating the architecture of complex networks, which might be otherwise hard to grasp.
