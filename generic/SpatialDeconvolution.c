...
#define TH_GENERIC_FILE "generic/SpatialDeconvolution.c"
#else
...
static int nn_(SpatialDeconvolution_updateOutput)(lua_State *L) {
  // Input
...
static int nn_(SpatialDeconvolution_updateGradInput)(lua_State *L) {
  // Inputs
  THTensor *input = (THTensor *)luaT_checkudata(L, 2, torch_Tensor);
...
static int nn_(SpatialDeconvolution_accGradParameters)(lua_State *L) {
  // Inputs
  THTensor *input = (THTensor *)luaT_checkudata(L, 2, torch_Tensor);
  THTensor *gradOutput = (THTensor *)luaT_checkudata(L, 3, torch_Tensor);
...
static const struct luaL_Reg nn_(SpatialDeconvolution__) [] = {
  {"SpatialDeconvolution_updateOutput", nn_(SpatialDeconvolution_updateOutput)},
  {"SpatialDeconvolution_updateGradInput", nn_(SpatialDeconvolution_updateGradInput)},
  {"SpatialDeconvolution_accGradParameters", nn_(SpatialDeconvolution_accGradParameters)},
  {NULL, NULL}
};

static void nn_(SpatialDeconvolution_init)(lua_State *L)
{
  luaT_pushmetatable(L, torch_Tensor);
  luaT_registeratname(L, nn_(SpatialDeconvolution__), "nn");
  lua_pop(L,1);
}
